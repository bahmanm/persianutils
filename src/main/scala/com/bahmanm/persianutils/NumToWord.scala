...
/** @author
  *   Amir Karimi (karimi AT dev-frame.com)
  * @author
...
object NumToWord {
  ...
  /** Converts the specified number to corresponding words.
    *
    * @deprecated
    *   Use [[cardinal]] instead. Scheduled for removal in version 7.0.0.
    */
  ...
  private final val tensWords =
    List("", "ده", "بیست", "سی", "چهل", "پنجاه", "شصت", "هفتاد", "هشتاد", "نود")
  private final val hundredsWords = List(
    "",
    "صد",
    "دویست",
    "سیصد",
  ...
  private final val thousandPowerWords =
    List("", "هزار", "میلیون", "میلیارد", "هزار میلیارد")

  /** Converts the specified number to corresponding words considering the
    * specified thousand power.
    */
  private def numToWords(n: Long, powerOfThousand: Int): String = n match {
    case 0 => ""
    case _ =>
      joinWords(
        numToWords(n / 1000, powerOfThousand + 1),
        hundredsToWords((n % 1000).toInt, powerOfThousand)
      )
  }
  ...
  /** Joins the number word and thousand power word together in such way that no
    * extra space and power word is produced.
    */
  private def joinThousandWords(prefix: String, powerWord: String) =
    prefix match {
      case "" => ""
  ...
  /** Joins strings using the specified separator. It doesn't produce extra
    * separator in certain situations ex. When there is just one string to be
    * joined.
    */
  private def joinWords(words: String*)(implicit separator: String = " و ") =
    words.filter(_.nonEmpty).mkString(separator)
}
