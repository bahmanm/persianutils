...
class DateToWordSmokeTest extends munit.FunSuite {
  ...
  test("PersianDate toWords") {
    ...
    assertEquals(
      DateToWord.persian(1402, 1, 1),
      "یکم فروردین یک هزار و چهار‌صد و دو"
    )
  }
  ...
  test("GregorianDate toWords") {
    ...
    assertEquals(
      DateToWord.gregorian(2023, 3, 21),
      "بیست و یکم مارس دو هزار و بیست و سه"
    )
    assertEquals(
      DateToWord.gregorian(LocalDate.of(2023, 3, 21)),
      "بیست و یکم مارس دو هزار و بیست و سه"
    )
  }
